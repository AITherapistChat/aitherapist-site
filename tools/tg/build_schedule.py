...
⚠️ Время публикации у Телеграма своё (TG_TIME) и с вэкашным планом не совпадает.
ВК постит из собственной отложки — точно в 19:00, как и задумано. Телеграм постит
кроном GitHub Actions, а тот опаздывает и роняет слоты: 28.08.2026 пост, стоявший
на 19:00, вышел в 23:46. Вечернее время не оставляет запаса — опоздание сразу
уводит пост в ночь. Днём запас есть: до конца окна крона (19:47 МСК) остаётся
больше семи часов повторных попыток, и любая из них — всё ещё нормальное время.
"""
...
# Единое время публикации в канале — см. предупреждение в шапке.
# Время из PLAN здесь намеренно игнорируется: оно вэкашное.
TG_TIME = '12:00'
